...
Widget collapsibleSectionUseCase(BuildContext context) => _pad(
      CollapsibleSection(
        ...
        summary:
            context.knobs.stringOrNull(label: 'Summary', initialValue: null),
        child: const Padding(
...
Widget sectionTitleUseCase(BuildContext context) => _pad(
      SectionTitle(
        ...
        subtitle: context.knobs.stringOrNull(
            label: 'Subtitle', initialValue: 'Subtitle'),
      ),
    );
...
Widget compactStatRowUseCase(BuildContext context) => _pad(
      CompactStatRow(
        ...
        labelEN: 'Athletics',
        modifier:
            context.knobs.string(label: 'Modifier', initialValue: '+5'),
        isProficient:
            context.knobs.boolean(label: 'Proficient', initialValue: true),
      ),
    );
...
Widget goldPipsUseCase(BuildContext context) => _pad(
      GoldPips(
        current:
            context.knobs.int.slider(label: 'Current', initialValue: 3, min: 0, max: 6),
        max: context.knobs.int.slider(label: 'Max', initialValue: 4, min: 1, max: 6),
      ),
    );
