...
SkillDef skillByName(String name) =>
    kSkills.firstWhere((s) => s.name == name);
...
const _allSkillNames = [
  '體能', '特技', '巧手', '隱匿', '奧秘', '歷史', '調查', '自然', '宗教',
  '馴獸', '洞察', '醫藥', '感知', '求生', '欺瞞', '威嚇', '表演', '說服',
];

const kClasses = <ClassOption>[
  ClassOption(
    cn: '野蠻人', en: 'Barbarian', hitDie: 12, saves: ['STR', 'CON'],
    primaryAbilities: ['STR'], skillCount: 2,
    skillChoices: ['馴獸', '體能', '威嚇', '自然', '感知', '求生'],
    description: '〔敘述佔位〕揮舞重武器、以怒火為力量的前線戰士；越受傷越兇猛。（文案待補）'),
  ClassOption(
    cn: '吟遊詩人', en: 'Bard', hitDie: 8, spellAbility: 'CHA',
    saves: ['DEX', 'CHA'], primaryAbilities: ['CHA'], skillCount: 3,
    skillChoices: _allSkillNames,
    description: '〔敘述佔位〕以表演與魔法鼓舞同伴、樣樣通的多才施法者。（文案待補）'),
  ClassOption(
    cn: '牧師', en: 'Cleric', hitDie: 8, spellAbility: 'WIS',
    saves: ['WIS', 'CHA'], primaryAbilities: ['WIS'], skillCount: 2,
    skillChoices: ['歷史', '洞察', '醫藥', '說服', '宗教'],
    description: '〔敘述佔位〕承神祇之力的神聖施法者，能治療、守護與打擊。（文案待補）'),
  ClassOption(
    cn: '德魯伊', en: 'Druid', hitDie: 8, spellAbility: 'WIS',
    saves: ['INT', 'WIS'], primaryAbilities: ['WIS'], skillCount: 2,
    skillChoices: ['奧秘', '馴獸', '洞察', '醫藥', '自然', '感知', '宗教', '求生'],
    description: '〔敘述佔位〕汲取自然之力的施法者，可變身野獸。（文案待補）'),
  ClassOption(
    cn: '戰士', en: 'Fighter', hitDie: 10, saves: ['STR', 'CON'],
    primaryAbilities: ['STR', 'DEX'], skillCount: 2,
    skillChoices: ['特技', '馴獸', '體能', '歷史', '洞察', '威嚇', '說服', '感知', '求生'],
    description: '〔敘述佔位〕精通各式武器與戰技的全能戰鬥者。（文案待補）'),
  ClassOption(
    cn: '武僧', en: 'Monk', hitDie: 8, saves: ['STR', 'DEX'],
    primaryAbilities: ['DEX', 'WIS'], skillCount: 2,
    skillChoices: ['特技', '體能', '歷史', '洞察', '宗教', '隱匿'],
    description: '〔敘述佔位〕以氣與徒手武技作戰、身法迅捷的武者。（文案待補）'),
  ClassOption(
    cn: '聖騎士', en: 'Paladin', hitDie: 10, spellAbility: 'CHA',
    saves: ['WIS', 'CHA'], primaryAbilities: ['STR', 'CHA'], skillCount: 2,
    skillChoices: ['體能', '洞察', '威嚇', '醫藥', '說服', '宗教'],
    description: '〔敘述佔位〕立下誓言、半戰半法的聖戰士，能治療與制裁。（文案待補）'),
  ClassOption(
    cn: '遊俠', en: 'Ranger', hitDie: 10, spellAbility: 'WIS',
    saves: ['STR', 'DEX'], primaryAbilities: ['DEX', 'WIS'], skillCount: 3,
    skillChoices: ['馴獸', '體能', '洞察', '調查', '自然', '感知', '隱匿', '求生'],
    description: '〔敘述佔位〕通曉荒野、半法的追蹤獵手與神射手。（文案待補）'),
  ClassOption(
    cn: '盜賊', en: 'Rogue', hitDie: 8, saves: ['DEX', 'INT'],
    primaryAbilities: ['DEX'], skillCount: 4,
    skillChoices: ['特技', '體能', '欺瞞', '洞察', '威嚇', '調查', '感知', '表演', '說服', '巧手', '隱匿'],
    description: '〔敘述佔位〕擅長潛行、技巧與偷襲的多面手。（文案待補）'),
  ClassOption(
    cn: '術士', en: 'Sorcerer', hitDie: 6, spellAbility: 'CHA',
    saves: ['CON', 'CHA'], primaryAbilities: ['CHA'], skillCount: 2,
    skillChoices: ['奧秘', '欺瞞', '洞察', '威嚇', '說服', '宗教'],
    description: '〔敘述佔位〕天生擁有魔法血脈、爆發力強的施法者。（文案待補）'),
  ClassOption(
    cn: '邪術師', en: 'Warlock', hitDie: 8, spellAbility: 'CHA',
    saves: ['WIS', 'CHA'], primaryAbilities: ['CHA'], skillCount: 2,
    skillChoices: ['奧秘', '欺瞞', '歷史', '威嚇', '調查', '自然', '宗教'],
    description: '〔敘述佔位〕與異界主宰締約、換取詭祕力量的施法者。（文案待補）'),
  ClassOption(
    cn: '法師', en: 'Wizard', hitDie: 6, spellAbility: 'INT',
    saves: ['INT', 'WIS'], primaryAbilities: ['INT'], skillCount: 2,
...
const kBackgrounds = <BackgroundOption>[
  BackgroundOption(
    cn: '士兵', en: 'Soldier', abilities: ['STR', 'DEX', 'CON'],
    skills: ['體能', '威嚇'], originFeat: '野蠻打擊',
    description: '〔敘述佔位〕受過軍事訓練的戰士，習於紀律與陣列作戰。（文案待補）'),
  BackgroundOption(
    cn: '賢者', en: 'Sage', abilities: ['CON', 'INT', 'WIS'],
    skills: ['奧秘', '歷史'], originFeat: '法術新手（法師）',
    description: '〔敘述佔位〕埋首典籍的學者，博聞強記、通曉祕辛。（文案待補）'),
  BackgroundOption(
    cn: '侍僧', en: 'Acolyte', abilities: ['INT', 'WIS', 'CHA'],
    skills: ['洞察', '宗教'], originFeat: '法術新手（牧師）',
    description: '〔敘述佔位〕在神殿中侍奉的信徒，熟稔禮儀與信仰。（文案待補）'),
  BackgroundOption(
    cn: '罪犯', en: 'Criminal', abilities: ['DEX', 'CON', 'INT'],
    skills: ['巧手', '隱匿'], originFeat: '警覺',
    description: '〔敘述佔位〕遊走法外的能手，熟悉黑街與不法門路。（文案待補）'),
  BackgroundOption(
    cn: '守衛', en: 'Guard', abilities: ['STR', 'INT', 'WIS'],
    skills: ['體能', '感知'], originFeat: '警覺',
    description: '〔敘述佔位〕看守城門與要地的衛兵，警覺而盡責。（文案待補）'),
  BackgroundOption(
    cn: '嚮導', en: 'Guide', abilities: ['DEX', 'CON', 'WIS'],
    skills: ['隱匿', '求生'], originFeat: '法術新手（德魯伊）',
    description: '〔敘述佔位〕熟悉荒野路徑的引路人，能在野外生存。（文案待補）'),
  BackgroundOption(
    cn: '水手', en: 'Sailor', abilities: ['STR', 'DEX', 'WIS'],
    skills: ['特技', '感知'], originFeat: '酒館鬥毆',
    description: '〔敘述佔位〕走船闖海的水手，身手俐落、見多識廣。（文案待補）'),
  BackgroundOption(
    cn: '貴族', en: 'Noble', abilities: ['STR', 'INT', 'CHA'],
    skills: ['歷史', '說服'], originFeat: '技藝精湛',
