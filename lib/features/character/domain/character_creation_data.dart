...
/// 內建語言清單（保守收錄）。
///
/// 只收 **SRD 5.1 就存在、延續至 5.2 可能性高的 16 種**。刻意未收 2024 新增
/// 而 SRD 5.2 收錄與否未經查證的三種——通用手語、德魯伊語、盜賊黑話。
///
/// **內容庫沒有語言資料**（races／backgrounds／classes／feats 皆無語言名稱），
/// 無從查證，而 content-scope 規定規則內容僅能來自 SRD 5.2 或使用者自訂：
/// 多收可能違規，少收只是使用者多打幾個字，且自由填寫已涵蓋。查證後補進此
/// 清單即可，既有資料不受影響（語言就是字串）。
///
/// 不分標準／稀有：兩者的差別在建角時的取得難度，而本清單服務的是冒險中取得。
