...
- **短休流程（互動視窗）**
  - **生命骰 Hit Dice**：花費生命骰擲骰回復 HP（顯示剩餘/最大，可選擇花幾顆）
  - **奧術恢復 Arcane Recovery**（法師，1/天）：回補總環數 ≤ 角色等級一半的法術位（不超過 5 環）
  - 其他「休息時才使用」的職業能力（依職業動態顯示）
  - 回復「短休資源」（已於 class-resources 做 `shortRest()`，此處整合進流程）
- **長休流程**
  - 完整恢復：HP 回滿、法術位回滿、職業資源回滿、臨時 HP 清空（部分已於前述 change 完成）
  - 生命骰回復一半（D&D 規則）
  - 力竭等級 −1（2024 長休遞減）
